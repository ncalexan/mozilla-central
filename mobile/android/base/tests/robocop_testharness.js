...
function _evalURI(aURI, sandbox) {
  ...
  let uri = SpecialPowers.Services.io
        .newURI(aURI, window.document.characterSet, baseURI);

  req.open('GET', uri.spec, false);
  req.send();

  return SpecialPowers.Cu.evalInSandbox(req.responseText, sandbox, "1.8", aURI, 1);
}

/**
 * Execute the Javascript file at aURI in a testing sandbox populated
 * with the Javascript test harness.
 *
 * aURI should be a String, relative (to window.document.baseURI) or
 * absolute.
 *
 * The Javascript test harness sends all output to Java via
 * Robocop:Status messages.
 */
function testOneFile(aURI) {
  ...
  return _evalURI(aURI, testScope);
}
